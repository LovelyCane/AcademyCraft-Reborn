...
import cn.academy.internel.ui.NetDelegateJava;
...
public class SkillTreeJava {
    ...
    public static Widget initialize(IDeveloper developer, CGui gui) {
        ...
        if (!aData.hasCategory()) {
            ...
            devData.reset();
        } else if (Optional.of(player.getHeldItem(EnumHand.MAIN_HAND)).map(item -> item.getItem() == ACItems.magnetic_coil).orElse(false)) {
        ...
        // Initialize machine panel
        {
            ...
            if (developer != null) {
                ...
                if (developer instanceof TileDeveloper) {
                    TileDeveloper tile = (TileDeveloper) developer;
                    send(NetDelegateJava.MSG_GET_NODE, tile, Future.create((String result) -> panel.getWidget("button_wireless/text_nodename").getComponent(TextBox.class).setContent(result != null ? result : "N/A")));

                    panel.getWidget("button_wireless").listen(LeftClickEvent.class, () -> {
                        Widget wirelessPage = WirelessPage.userPage(tile).window.centered();
                        Widget cover = blackCover(gui);
                        cover.addWidget(wirelessPage);

                        cover.listen(LeftClickEvent.class, () -> cover.getComponent(Cover.class).end());
                        cover.listen(CloseEvent.class, () -> gui.postEvent(new RebuildEvent()));

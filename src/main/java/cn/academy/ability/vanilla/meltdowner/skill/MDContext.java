...
public class MDContext extends Context<Meltdowner> {
    ...
    private final float tickConsumption = MathUtils.lerpf(10, 15, exp);
    ...
    private void s_madeAlive() {
        float overload = MathUtils.lerpf(200, 170, exp);
    ...
    private void s_perform() {
        ...
        final double[] length = new double[1];
        RangedRayDamage rrd = new RangedRayDamage.Reflectible(ctx, MathUtils.lerpf(2, 3, exp), getEnergy(ct), reflector -> {
    ...
    private void s_reflected(Entity reflector) {
        ...
        if (result.typeOfHit == RayTraceResult.Type.ENTITY) {
            ctx.attack(result.entityHit, 0.5f * MathUtils.lerpf(20, 50, exp));
        }
    }
    ...
    private float getDamage(int ct) {
        return timeRate(ct) * MathUtils.lerpf(18, 50, exp);
    }

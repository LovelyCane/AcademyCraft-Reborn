...
public class ACItemAdditionalRegistry {
    ...
    private static final ResourceLocation[] DEFAULT_APPEARANCE = {LootTableList.CHESTS_ABANDONED_MINESHAFT, LootTableList.CHESTS_DESERT_PYRAMID, LootTableList.CHESTS_JUNGLE_TEMPLE, LootTableList.CHESTS_STRONGHOLD_LIBRARY, LootTableList.CHESTS_SIMPLE_DUNGEON};
    ...
    public static void addLoots(String poolName, int minValue, int maxValue, ACLootItem... entries) {
        ...
        for (ACLootItem item : entries) {
            LootEntry entry;
            if (item.item != null) {
                entry = new LootEntryItem(item.item, item.weight, item.quality, new LootFunction[]{item.func}, item.conds, item.name);
            } else {

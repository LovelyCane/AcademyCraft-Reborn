...
public class TileMetalFormer extends TileReceiverBase implements ISidedInventory {
    ...
    public enum Mode {
        ...
        Mode() {
            texture = new ResourceLocation("academy:textures/guis/icons/icon_former_" + this.toString().toLowerCase() + ".png");
        }
    }
        ...
        if (!world.isRemote) {
            if (current != null) {
                ...
                if (!isActionBlocked() && this.pullEnergy(CONSUME_PER_TICK) == CONSUME_PER_TICK) {
                    ...
                    if (workCounter == WORK_TICKS) { // Finish the job.
                        ...
                        if (inputSlot.getCount() == 0) this.setInventorySlotContents(SLOT_IN, ItemStack.EMPTY);

                        if (!outputSlot.isEmpty()) outputSlot.grow(current.output.getCount());
                        else this.setInventorySlotContents(SLOT_OUT, current.output.copy());
            ...
            /* Process energy in/out */
            {
                ...
                if (!stack.isEmpty() && EnergyItemHelper.isSupported(stack)) {
                    double gain = EnergyItemHelper.pull(stack, Math.min(getMaxEnergy() - getEnergy(), getBandwidth()), false);
    ...
    // SERVER only
    private void sync() {
        NetworkMessage.sendToAllAround(TargetPoints.convert(this, 12), this, "sync", workCounter, current, mode);
    }
    ...
    private boolean isActionBlocked() {
        ...
        return !(current.accepts(inputSlot, mode) && (outputSlot.isEmpty() || (outputSlot.getItem() == current.output.getItem() && outputSlot.getItemDamage() == current.output.getItemDamage() && outputSlot.getCount() + current.output.getCount() <= outputSlot.getMaxStackSize())));
    }
    ...
    private void updateSounds() {
        if (sound != null && !isWorkInProgress()) {
            ...
            sound = new TileEntitySound(this, "machine.machine_work").setLoop().setVolume(.6f);

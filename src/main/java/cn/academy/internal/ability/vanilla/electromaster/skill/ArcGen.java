...
public class ArcGen extends Skill {
    ...
    public static class ArcGenContext extends Context<ArcGen> {
        ...
        final IBlockSelector blockFilter = (world, x, y, z, block) -> block == Blocks.WATER || block == Blocks.FLOWING_WATER || BlockSelectors.filNormal.accepts(world, x, y, z, block);
                    ...
                    if (result.typeOfHit == RayTraceResult.Type.ENTITY) {
                        ...
                        if (block == Blocks.WATER) {
                            if (RandUtils.ranged(0, 1) < fishProb) {
                                world.spawnEntity(new EntityItem(world, result.hitVec.x, result.hitVec.y, result.hitVec.z, new ItemStack(Items.COOKED_FISH)));

...
import cn.academy.internal.ability.context.IStateProvider;
import cn.academy.internal.util.Plotter;
...
public class GroundshockContext extends Context<Groundshock> implements IConsumptionProvider, IStateProvider {
    ...
    public void s_perform() {
        if (player.onGround && consume()) {
            ...
            ArrayList<BlockPos> list = new ArrayList<>(dejavuBlocks);
            sendToClient(MSG_PERFORM, list);
        }
        terminate();
    }

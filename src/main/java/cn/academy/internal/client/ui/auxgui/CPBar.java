...
public class CPBar extends Widget {
...

    {
        ...
        for (int i = 0; i < iteration; ++i) {
            ...
            frame.time = sum;
            frame.direction = new Vector2d(Math.sin(theta) * offsetNorm * offsetMax * aspect, Math.cos(theta) * offsetNorm * offsetMax);
            frames.add(frame);
    ...
    private void initEvents() {
        listen(FrameEvent.class, (w, e) -> {
            ...
            mAlpha = (time - showTime < BLENDIN_TIME) ? (float) (time - showTime) / BLENDIN_TIME : (active ? 1.0f : Math.max(0.0f, 1 - (time - lastDrawTime) / 200.0f));
            ...
            if (interf) {
                ...
                mAlpha *= (float) alphaCurve.valueAt(timeInput);
            }
            ...
            if (mAlpha > 0) {
                ...
                /* Draw CPBar */
                {
                    ...
                    if (estmCons != 0) {
                        ...
                        mAlpha *= (float) (0.2f + 0.1f * (1 + Math.sin(time / 80.0f)));
                ...
                // Draw data
                {
                    ...
                    if (alpha > 0) {
                        ...
                        float len10 = font.getTextWidth(str10, option), len11 = font.getTextWidth(str11, option), len20 = font.getTextWidth(str20, option), len21 = font.getTextWidth(str21, option);
    ...
    private void drawCPBar(float prog, boolean cantuse) {
        ...
        double texWidth = GL11.glGetTexLevelParameteri(GL11.GL_TEXTURE_2D, 0, GL11.GL_TEXTURE_WIDTH), texHeight = GL11.glGetTexLevelParameteri(GL11.GL_TEXTURE_2D, 0, GL11.GL_TEXTURE_HEIGHT);
        t.startDrawingQuads();
    ...
    final Color CRL_P_BACK = new Color(48, 48, 48, 160), CRL_P_FORE = new Color(255, 255, 255, 200);
    ...
    private void drawPresetHint(double progress, long untilLast) {
        ...
        int cur = PresetData.get(Minecraft.getMinecraft().player).getCurrentID();
    ...
    static final Color CRL_KH_BACK = new Color(65, 65, 65, 70), CRL_KH_GLOW = new Color(255, 255, 255, 40);

...
public class PresetEditUI extends GuiScreen {
    ...
    static final Color CRL_BACK = new Color(49, 49, 49, 200), CRL_WHITE = Colors.fromFloat(1, 1, 1, 0.6f), CRL_GLOW = Colors.fromFloat(1, 1, 1, 0.2f);

    static WidgetContainer loaded = CGUIDocument.read(new ResourceLocation("academy:guis/preset_edit.xml"));
    static Widget template = loaded.getWidget("template");
    ...
    private class TransitPage extends Page {
        ...
        public TransitPage(int _id) {
            ...
            listen(FrameEvent.class, (w, e) -> DrawTexture.get(w).color.setAlpha(alpha));
        }
    ...
    private static class AlphaAssign implements IGuiEventHandler<FrameEvent> {
        @Override
    ...
    private class Selector extends Widget {
        ...
        public Selector(int _keyid) {
            ...
            width = available.size() < MAX_PER_ROW ? MARGIN * 2 + SIZE + STEP * (providers.size() - 1) : MARGIN * 2 + SIZE + STEP * (MAX_PER_ROW - 1);

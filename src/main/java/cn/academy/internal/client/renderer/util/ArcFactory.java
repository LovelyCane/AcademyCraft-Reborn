...
public class ArcFactory {
    ...
    public static class Arc {
        ...
        private void handleSegment(List<Segment> list, Vec3d normal, double len) {
            Vec3d lastDir = null;
            for (Segment s : list) {
                if (s.start.pt.x > len) break;

                Vec3d dir = randomRotate(15, crossProduct(subtract(s.end.pt, s.start.pt), normal)).normalize();
                if (lastDir == null) lastDir = dir;

                Vec3d p1 = add(s.start.pt, multiply(lastDir, s.start.width)), p2 = add(s.start.pt, multiply(lastDir, -s.start.width)), p3 = add(s.end.pt, multiply(dir, s.end.width)), p4 = add(s.end.pt, multiply(dir, -s.end.width));

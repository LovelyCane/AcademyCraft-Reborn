...
public class TileImagPhaseRenderer extends TileEntitySpecialRenderer<TileImagPhase> {
    ...
    public TileImagPhaseRenderer() {
        t = Tessellator.instance;
        layers = Resources.getEffectSeq("imag_proj_liquid", 3);

        _mGetFluidHeight = ReflectionUtils.getObfMethod(BlockFluidRenderer.class, "getFluidHeight", "func_178269_a", IBlockAccess.class, BlockPos.class, Material.class);
    }
    ...
    @Override
    public void render(TileImagPhase te, double x, double y, double z, float w, int destroyStage, float alpha_) {
        ...
        double ht = 1.2 * Math.sqrt(getFluidHeight(te.getWorld(), p, te.getWorld().getBlockState(p).getMaterial()));

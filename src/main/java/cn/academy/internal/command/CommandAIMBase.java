...
public abstract class CommandAIMBase extends ACCommand {
    ...
    String[] commands = {"help", "cat", "catlist", "learn", "learn_all", "reset", "learned", "skills", "fullcp", "level", "exp", "cd_clear", "maxout"};
    ...
    protected void matchCommands(ICommandSender ics, EntityPlayer player, String[] pars) {
        ...
        switch (pars[0]) {
            ...
            case "cat": {
                if (pars.length == 1) {
                    sendChat(ics, getLoc("curcat"), aData.hasCategory() ? aData.getCategory().getDisplayName() : I18n.translateToLocal(getLoc("nonecat")));
                    return;
    ...
    private Skill tryParseSkill(Category cat, String str) {
        ...
        if (i != null) return cat.getSkill(i);

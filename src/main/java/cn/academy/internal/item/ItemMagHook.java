...
public class ItemMagHook extends Item {
    ...
    private void initClient() {
        _modelLocation = new ModelResourceLocation(Objects.requireNonNull(getRegistryName()), "inventory");
    ...
    public void onModelBake(ModelBakeEvent ev) {
        ...
        Matrix4f fpTrans = new TransformChain().scale(1.4f).rotate(0, 90, 180).translate(0f, .5f, 0.4f).build();

        Matrix4f fpTransLeft = new TransformChain(fpTrans).translate(1.4f, 0f, 0f).build();

        Matrix4f tpTrans = new TransformChain().rotate(0, 90, 180).scale(.8f).translate(-.4f, .5f, .7f).build();

        Matrix4f tpTransLeft = new TransformChain(tpTrans).translate(0.9f, 0f, 0f).build();
        ...
        bakedModel.mapTransform(TransformType.GROUND, new TransformChain().rotate(0, 90, 180).translate(-.4f, .9f, .7f).scale(.5f).build());
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
        ...
        world.playSound(player, player.posX, player.posY, player.posZ, SoundEvents.ENTITY_EGG_THROW, SoundCategory.PLAYERS, 0.5F, 0.4F / (itemRand.nextFloat() * 0.4F + 0.8F));
        if (!world.isRemote) {

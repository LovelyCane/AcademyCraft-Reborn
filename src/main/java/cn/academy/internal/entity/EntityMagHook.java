...
public class EntityMagHook extends EntityAdvanced {
    ...
    private void dropAsItem() {
        getEntityWorld().spawnEntity(new EntityItem(getEntityWorld(), posX, posY, posZ, new ItemStack(AcademyCraftItemList.MAG_HOOK)));
        setDead();
    }
    ...
    public void preRender() {
        if (this.isHit) {
            ...
            setPosition(hookX + 0.5 + hitSide.getDirectionVec().getX() * 0.51, hookY + 0.5 + hitSide.getDirectionVec().getY() * 0.51, hookZ + 0.5 + hitSide.getDirectionVec().getZ() * 0.51);
        }
    }
}
...
import cn.academy.internal.ability.vanilla.electromaster.skill.MagManip;
...
public class MagManipEntityBlock extends EntityBlock {
    ...
    public void onFirstUpdate() {
        ...
        if (world.isRemote) {
            ...
            NetworkMessage.sendToAllAround(new NetworkRegistry.TargetPoint(world.provider.getDimension(), posX, posY, posZ, 40), this, "MSG_ENT_SYNC", player2.getEntityId(), actionType, tx, ty, tz);
        }
    }
    ...
    private void startClient() {
        ...
        surrounder.addMotionHandler(new MotionHandler() {
            ...
            @Override
            public void onUpdate() {
                if (MagManipEntityBlock.this.isDead)
                    getTarget().setDead();
            }
        });

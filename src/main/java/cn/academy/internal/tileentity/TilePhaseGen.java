...
public class TilePhaseGen extends TileGeneratorBase implements IFluidHandler {
    ...
    // 废话~~~
    public static final int SLOT_LIQUID_IN = ContainerPhaseGen.SLOT_LIQUID_IN, SLOT_LIQUID_OUT = ContainerPhaseGen.SLOT_LIQUID_OUT, SLOT_OUTPUT = ContainerPhaseGen.SLOT_OUTPUT;
    ...
    public void update() {
        ...
        if (!getWorld().isRemote) {
            ...
            { // Sink in liquid
                ...
                if (!stack.isEmpty() && isPhaseLiquid(stack) && isOutputSlotAvailable() && (getTankSize() - getLiquidAmount() > PER_UNIT)) {
                    ...
                    if (!output.isEmpty()) {
                        ...
                        this.setInventorySlotContents(SLOT_LIQUID_OUT, AcademyCraftItemList.EMPTY_MATTER_UNIT.getDefaultInstance());
                    }
                }
            }
    ...
    private boolean isPhaseLiquid(ItemStack stack) {
        return stack.getItem() == AcademyCraftItemList.MATTER_UNIT_PHASE_LIQUID;
    }

    private boolean isOutputSlotAvailable() {
        ItemStack stack = getStackInSlot(SLOT_LIQUID_OUT);
        return stack.isEmpty() || (stack.getItem() == AcademyCraftItemList.EMPTY_MATTER_UNIT && stack.getCount() < stack.getMaxStackSize());
    }
}